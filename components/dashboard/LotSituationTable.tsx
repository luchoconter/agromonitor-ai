...
import { CheckSquare, Square, FileText, X, AlertTriangle, CheckCircle2, User as UserIcon, Calendar, MapPin, Play, Pause, FlaskConical, List, ClipboardList, ArrowLeft, MessageSquare, Clock, DollarSign, Save, Mic, StopCircle, Eye, PenLine, ShieldCheck, Tractor, Sprout, Bug, ImageIcon } from 'lucide-react';
        ...
        <table className="w-full text-left border-collapse">
          <thead className="bg-gray-50 dark:bg-gray-900/50 text-gray-500 dark:text-gray-400 font-semibold uppercase text-xs">
            <tr>
              ...
              <th className="px-4 py-3 whitespace-nowrap">Lote / Cultivo</th>
              <th className="px-4 py-3 text-center whitespace-nowrap">
                  <div className="flex flex-col">
                      <span>Consumo / Total</span>
                      <span className="text-[9px] font-normal text-gray-400 normal-case">(Estimado)</span>
                  </div>
              </th>
              <th className="px-4 py-3 text-center whitespace-nowrap bg-gray-100/50 dark:bg-gray-800/50 border-l border-gray-200 dark:border-gray-700">
                  <div className="flex flex-col">
                      <span>Estado</span>
              ...
              <th className="px-4 py-3 text-center whitespace-nowrap bg-blue-50/30 dark:bg-blue-900/10 border-l-2 border-blue-100 dark:border-blue-900/30 text-blue-700 dark:text-blue-300">
                  <div className="flex flex-col items-center">
                      <span className="flex items-center gap-1"><CheckSquare className="w-3 h-3"/> Ejecutada</span>
                      <span className="text-[9px] font-normal opacity-70 normal-case">(Cliente)</span>
                  </div>
              </th>
              <th className="px-4 py-3 text-center whitespace-nowrap bg-blue-50/30 dark:bg-blue-900/10 text-blue-700 dark:text-blue-300">
                  <div className="flex flex-col items-center">
                      <span className="flex items-center gap-1"><MessageSquare className="w-3 h-3"/> Nota Cliente</span>
                      <span className="text-[9px] font-normal opacity-70 normal-case">(Cliente)</span>
                  </div>
              </th>
            </tr>
          ...
          <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
            {plots.map(plot => {
              ...
              const latestMonitoring = plotMonitorings.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime())[0];
              const phenology = latestMonitoring?.phenology;

              const activePrescription = prescriptions
              ...
              return (
                <tr key={plot.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/20 transition-colors">
                          ...
                          {phenology && (
                              <>
                                ...
                                <span className={`px-1.5 rounded-sm font-bold ${
                                    phenology.startsWith('V') ? 'text-green-600 bg-green-50 dark:bg-green-900/30' : 
                  ...
                  <td className="px-4 py-3 text-center align-middle bg-gray-50/30 dark:bg-gray-800/30">
                      {activePrescription ? (
                          <button 
                            ...
                            className="p-2 bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 rounded-lg hover:bg-blue-100 dark:hover:bg-blue-900/40 transition-colors mx-auto"
                  ...
                  <td className="px-4 py-3 text-center align-middle border-l-2 border-blue-50 dark:border-blue-900/20 bg-blue-50/10 dark:bg-blue-900/5">
                      {activePrescription ? (
                          <div className="flex flex-col items-center">
                              <button 
                                onClick={() => handleExecutionToggle(activePrescription, plot.id, execution.executed, execution.observation || '')}
                                disabled={isEngineer}
                                className={`flex items-center justify-center transition-colors ${execution.executed ? 'text-green-600 dark:text-green-400' : 'text-gray-300 dark:text-gray-600 hover:text-gray-500'} ${isEngineer ? 'cursor-default opacity-80' : ''}`}
                                title={isEngineer ? "Modificable por el cliente" : "Marcar como ejecutada"}
                              >
                                  {execution.executed ? <CheckSquare className="w-6 h-6" /> : <Square className="w-6 h-6" />}
                              </button>
                              
                              {execution.executed && (
                                  <div className="flex flex-col items-center mt-0.5">
                                      {execution.executedAt && (
                                          <span className="text-[10px] text-green-700 dark:text-green-400 font-bold leading-none">
                                              {formatShortDate(execution.executedAt)}
                                          </span>
                                      )}
                                      {execution.executedBy && (
                                          <span className="text-[9px] text-gray-500 dark:text-gray-400 leading-tight max-w-[80px] truncate mt-0.5" title={execution.executedBy}>
                                              {execution.executedBy}
                                          </span>
                                      )}
                                  </div>
                  ...
                  <td className="px-4 py-3 text-center align-middle bg-blue-50/10 dark:bg-blue-900/5">
                      {activePrescription ? (
                          <button 
                            onClick={() => setObservationModal({
                                prescription: activePrescription,
                                plotId: plot.id,
                                text: execution.observation || '',
                                executed: execution.executed,
                                lastModifiedBy: execution.executedBy, 
                                lastModifiedAt: execution.executedAt,
                                readOnly: isEngineer
                            })}
                            className={`p-2 rounded-lg transition-colors inline-flex items-center justify-center relative ${
                                execution.observation 
                                    ? 'text-blue-600 bg-blue-50 hover:bg-blue-100 dark:bg-blue-900/30 dark:text-blue-300' 
                                    : (isEngineer ? 'text-gray-200 dark:text-gray-800' : 'text-gray-400 hover:text-gray-600 hover:bg-gray-100 dark:hover:bg-gray-800')
                            }`}
                            title={
                                execution.observation 
                                    ? (isEngineer ? "Leer nota del cliente" : "Editar nota") 
                                    : (isEngineer ? "Sin notas del cliente" : "Dejar nota al Ingeniero")
                            }
                            disabled={isEngineer && !execution.observation}
                          >
                              {isEngineer ? <Eye className="w-5 h-5" /> : (execution.observation ? <MessageSquare className="w-5 h-5 fill-current" /> : <PenLine className="w-5 h-5" />)}
                              {execution.observation && (
                                  <span className="absolute top-1 right-1 w-2 h-2 bg-blue-600 rounded-full border border-white dark:border-gray-800"></span>
                              )}
                          </button>
                      ) : (
                      ...
                      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-3 space-y-2">
                          <div className="flex justify-between items-center mb-1">
                              ...
                              <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase border ${
                                  selectedSummary.status === 'verde' ? 'bg-green-50 text-green-700 border-green-200' :
                      ...
                      {isEngineer ? (
                              ...
                              <div className="grid grid-cols-3 gap-2">
                                  {['verde', 'amarillo', 'rojo'].map((s) => (
                                      <button 
                                          ...
                                          className={`py-1.5 rounded-md text-xs font-bold uppercase border-2 transition-all ${
                                              feedbackStatus === s 
                          ...
                          <div className="bg-gray-50 dark:bg-gray-800 p-3 rounded-lg border border-gray-200 dark:border-gray-700">
                              <div className="flex items-center justify-between mb-2">
                                  <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase border ${
                                      selectedSummary.engineerStatus === 'verde' ? 'bg-green-100 text-green-800 border-green-200' :
