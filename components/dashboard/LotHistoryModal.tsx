...
import { X, Bug, FileText, CheckCircle2, Flag, Calendar, User, Play, Pause, Clock, AlertTriangle, Droplets } from 'lucide-react';
...
export const LotHistoryModal: React.FC<LotHistoryModalProps> = ({ plotId, onClose, data }) => {
    ...
    const historyEvents = useMemo(() => {
        ...
        // 1. Monitorings - REMOVED TO REDUCE CLUTTER
        // User requested to hide individual samples from this high-level log.
        ...
        // 3. Prescriptions (Recipes) & Applications
                                    ...
                                    <div className="bg-white dark:bg-gray-800 p-4 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm relative hover:shadow-md transition-shadow">
                                            ...
                                            <div>
                                                <span className={`text-xs font-bold uppercase tracking-wider mb-1 block
                                                    ${event.type === 'summary' ? (event.data.status === 'rojo' ? 'text-red-600' : event.data.status === 'amarillo' ? 'text-yellow-600' : 'text-green-600') :
                                                      event.type === 'prescription_created' ? 'text-blue-600' : 'text-emerald-600'}`}>
                                                    {event.type === 'summary' ? `Cierre de Lote (${event.data.status})` :
                                                     event.type === 'prescription_created' ? 'Receta Generada' : 'Aplicación Confirmada'}
                                                </span>
                                        ...
                                        {event.type === 'summary' && (
                                            <div className="text-sm">
                                                {event.data.notes && <p className="text-gray-700 dark:text-gray-300 mb-2">"{event.data.notes}"</p>}
                                        ...
                                        {event.type === 'prescription_created' && (
                                            <div className="text-sm">
                                                ...
                                                {event.data.notes && <p className="text-xs italic text-gray-500">"{event.data.notes}"</p>}
                                                {event.data.audioUrl && (
                                                    <button onClick={() => toggleAudio(event.data.audioUrl, `pre-${event.id}`)} className="mt-2 text-xs flex items-center bg-gray-100 px-2 py-1 rounded"><Play className="w-3 h-3 mr-1"/> Escuchar Indicaciones <audio id={`history-audio-pre-${event.id}`} src={event.data.audioUrl} className="hidden"/></button>
                                                )}
                                            </div>
                                        )}
                                        ...
                                        {event.type === 'application' && (
                                            <div className="text-sm">
                                                ...
                                                <p className="text-xs text-gray-400 mt-2">Corresponde a receta del {formatDate(event.data.prescription.date)}</p>
                                            </div>
                                        )}
